...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
        for (int i = 0; i < lastRankFilled; i++) {
            if (Math.abs(i - lastRankFilled) == Math.abs(filesWithQueens[i]-filesWithQueens[lastRankFilled])
                || filesWithQueens[i] == filesWithQueens[lastRankFilled]) {
                return true;
            }
        }
        return false;
